...
For diagrams, use text-based formats that render in Markdown:
- **Mermaid** for flowcharts, sequence diagrams, ER diagrams (supported by GitHub, Notion, many blog platforms)
- **ASCII art** as a fallback when Mermaid isn't available
- Describe what visuals (screenshots, benchmark charts) should look like so the author can create them

**Example Mermaid architecture diagram:**
````markdown
```mermaid
graph LR
  Client["React App"] --> CDN["CDN Cache"]
  CDN --> API["Node.js API"]
  API --> DB["PostgreSQL"]
```
````
...
sequenceDiagram
  ...
  CDN-->>Client: 200 (cache hit)
  CDN->>API: GET /resource (cache miss)
  API->>DB: SELECT ...
  DB-->>API: rows
  API-->>CDN: 200
  CDN-->>Client: 200
...
| Twitter/X | Thread summary + link | Summarize key points in 3-5 tweets |
| LinkedIn | Adapted version + link | More narrative, less code |
